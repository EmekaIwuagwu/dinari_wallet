...
const DinariRPC = (function() {
  ...
  let rpcEndpoint = 'https://52.241.1.239:8545';
  let requestId = 1;
   ...
   * @returns {Promise<Object>} Transaction details including hash
   */
    async function sendTransaction(transaction) {
      const result = await request('tx_send', transaction);
      // Handle both old format (txHash) and new format (tx object)
      if (result.tx) {
        return result.tx;
      }
  ...
  async function getTransaction(txHash) {
    ...
    /**
   * Get pending transactions by wallet address (from mempool)
   * @param {string} address
   * @param {number} limit - Max items to return (default: 20)
   * @returns {Promise<Object>} {transactions, total, returned, address}
   */
  async function getTransactionsByWallet(address, limit = 20) {
    return await request('tx_getByAddress', {
      address,
      limit
    });
  }
    ...
    /**
   * Get confirmed transaction history by wallet address (from blockchain)
   * @param {string} address
   * @param {number} limit - Max items to return (default: 20)
   * @returns {Promise<Object>} {transactions, total, returned, address, blocksScanned}
   */
  async function getTransactionHistory(address, limit = 20) {
    return await request('tx_getHistory', {
      address,
      limit
    });
  }
  ...
  async function buildAndSendTransaction(from, to, amount, tokenType, feeDNT, privateKey) {
    try {
      // Build RAW transaction object for blockchain to sign
      // The blockchain expects: from, to, amount, tokenType, fee, privateKey
      const tx = {
        from,
        to,
        amount,
        tokenType,
        fee: feeDNT,      // ✅ Changed from "feeDNT" to "fee"
        privateKey        // ✅ Include privateKey - blockchain will sign
      };

      // Send RAW transaction to blockchain (blockchain will sign it)
      const txData = await sendTransaction(tx);

      // Return hash (support both old and new response formats)
      return txData.hash || txData.txHash || txData;
    } catch (error) {
